							...
							<view v-for="(item,index) in orderList" :key="index" class="list-item">
								<view class="user-msg">
									<view class="user-avatar">
										<image :src="item.userInfo.userAvatar" mode="aspectFill"></image>
									</view>
								...
								<view class="order-price">
								   订单总价 : <span> {{item.orderPrice}}</span>
								</view>
								<view class="control-btn">
									<view class="btn-item item1" v-if="swiperCurrent==0">
										...
										<u-button class="btn" shape="circle" type="success" size="mini"
											@click="completeOrder(item.orderId)">完成订单</u-button>
									</view>
	...
	export default {
		...
		methods: {
			...
			async getOrderList() {
				...
				const res = await request('/users/GetChefOrderList', {
			...
			completeOrder(id){
				this.dialogShow = true;
				this.content = "是否已完成该订单？";
				this.currentOrderId = id;
				this.currentType=2;
			},
			//厨师-取消订单
			async cancleOrderAip(id) {
				const res = await request('/users/CancelChefOrder', {
			...
			async completeOrderApi(){
				const res = await request('/users/FinishChefOrder', {
					orderId: this.currentOrderId
				});
				if (res.code !== 200) {
					this.$refs.uToast.show({
